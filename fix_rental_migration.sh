...
echo "=== Fixing Failed Migration on promantis-rental ==="

# Step 1: Mark the failed migration as rolled back
echo "Step 1: Marking failed migration as rolled back..."
docker exec -it promantis-rental sh -c "cd apps/api && npx prisma migrate resolve --rolled-back 20260301000001_remove_ticket_confirmation"
...
if [ $? -eq 0 ]; then
    ...
    read -p "Enter your PostgreSQL container name (e.g., promantis-db-rental): " POSTGRES_CONTAINER
    
    # Manual fix in database
    docker exec -it $POSTGRES_CONTAINER psql -U promantis promantis_rental -c "
...
docker pull ghcr.io/sgpromantis/promantis:latest

# Step 3: Restart the container
echo "Step 3: Restarting promantis-rental..."
docker restart promantis-rental

# Step 4: Watch logs
echo "Step 4: Watching logs for migration..."
echo "Press Ctrl+C to exit logs once you see 'Connected to Prisma'"
docker logs promantis-rental -f
